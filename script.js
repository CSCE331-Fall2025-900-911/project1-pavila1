document.addEventListener("DOMContentLoaded", () => {
    ...
    // Get the About Me page element (assuming this script is also used on aboutme.html)
            ...
            if (navigationStartTime) {
                ...
                setTimeout(() => {
                    ...
                    if (document.getElementById("terminal-intro")) {
                        ...
                        // ⭐️ START TYPING EFFECT ON ABOUT ME PAGE LOAD (for direct access)
                        startTypingGreeting();
                    }
                }, timeToWait);
                    ...
                    if (isFirstVisit) {
                        ...
                        if (document.getElementById("terminal-intro")) {
                            showTerminal();
                             ...
                             // ⭐️ START TYPING EFFECT ON ABOUT ME PAGE LOAD (for first visit)
                            startTypingGreeting();
                        }
                    }
                }, 3000);
    ...
    // ----------------------------------------------------------------------
    // ⭐️ NEW TYPING FUNCTION FOR ABOUT ME PAGE
    // ----------------------------------------------------------------------
    function startTypingGreeting() {
        const textElement = document.getElementById('typing-greeting');
        
        // Only run if the element exists on the current page
        if (!textElement) return;

        // The full message to type out.
        const fullText = "Hello User!! My name is Paola Avila"; 
        let charIndex = 0;
        
        // Clear any previous text
        textElement.textContent = ''; 

        // Set up the cursor appearance
        textElement.style.borderRight = '3px solid var(--text-pink)'; 
        textElement.style.paddingRight = '5px'; 
        // Ensure the CSS animation for blinking is active
        ...
        function type() {
            if (charIndex < fullText.length) {
                ...
                // Typing complete: keep cursor blinking for a moment, then hide it
                setTimeout(() => {
    ...
    // ⭐️ Add a function to navigate to the About Me page and start the typing effect
    window.showAboutMePage = function () {
        showLoadingAndNavigate("aboutme.html");
    };
